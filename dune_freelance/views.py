...
def blogers(request):
    if request.method == 'POST':
        ...
        input_photo = request.FILES.get('input_photo')
        ...
        try:
            blog = models.Bloger(user=request.user, name_first=nickname, social_net=categr, subs=subs,tem=tem, photo_bloger=input_photo, url_social_net=url_soc)
            blog.save()
            return redirect('blogers')
        except:
...
def post(request):
    try:
        if request.method == 'POST':
            if 'input_data_post' in request.POST:
                ...
                photo_project = request.FILES.get('screenshot_add')


                posts_p = models.Posts.objects.get(id=idd)
                posts_p.data_konec = date_finish_proj
                posts_p.format = forma
                posts_p.work_url = url
                posts_p.kol_p += 1
                posts_p.photo = photo_project
    ...
    try:
        if request.method == 'POST':
            ...
            for _, row in df.iterrows():
                post_ex = models.Posts.objects.get(name_url=row['Название ссылки'])
                date_value = pd.to_datetime(row['Дата и время заявки / заказа'], format='%d.%m.%Y')
                result_get= models.result.objects.create(pst = post_ex, id_res = row['ID заявки'], id_off=row['ID Оффера'], name_off=row['Название ссылки'], status_off=row['Status'], date_off=date_value, summ_off=row['Сумма вознаграждения'])
                return redirect('result')
    except:
        messages.error(request, 'Произошла ошибка загрузки файла')
    ...
    try:
        if request.method == 'POST':
            if 'input_data_post' in request.POST:
                ...
                id_m = request.POST.get('idd')
                format_m = request.POST.get('format')
                fm = request.POST.get('finish_project')
                url_m = request.POST.get('url_input_project')
                photo_post = request.FILES.get('screenshot_add')
                aplic_m_p = models.Posts.objects.get(id=id_m)
                aplic_m_p.data_konec = fm
                aplic_m_p.format = format_m
                aplic_m_p.work_url = url_m
                aplic_m_p.kol_p += 1
                aplic_m_p.photo = photo_post
                aplic_m_p.save()
    except:
        messages.error(request, 'Не вспе поля заполнены')
...
def blogers_m(request):
    if request.method == 'POST':
        ...
        if models.Bloger.objects.filter(url_social_net=url_soc_m).exists():
            ...
            return redirect('blogers')
        try:
            blog_m = models.Bloger(user=request.user,tem=tem_m, name_first=nickname_m, social_net=categr_m, subs=subs_m, photo_bloger=input_photo,  url_social_net=url_soc_m)
            blog_m.save()
...
def projects_m(request):
    if request.method == 'POST':
        ...
        try:
            ...
            model = models.Projects_men(us_manager=request.user, name=name, men_email=email, pay=number, date_start=start, date_finish=finish, social_category=t_social, pay_category=type_pay, stat=status, nums=num_r)
            model.save()
            return redirect('projects_m')
        except:
